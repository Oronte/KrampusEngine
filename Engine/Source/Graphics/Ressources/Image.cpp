...
Krampus::Image::Image(const String& _path)
{
	image = sf::Image(std::filesystem::path(_path.StdString()));
}
...
Bool Krampus::Image::LoadFromFile(const String& _filename)
{
	return image.loadFromFile(std::filesystem::path(_filename.StdString()));
}
...
Bool Krampus::Image::SaveToFile(const String& _filename) const
{
	return image.saveToFile(std::filesystem::path(_filename.StdString()));
}
