...
public:
    // ── Constructors ─────────────────────────────────────────────────────────
    UInt() = default;
    UInt(int _o) : value((unsigned int)_o) {}
    UInt(unsigned int _o) : value(_o) {}
    UInt(float _o) : value((unsigned int)_o) {}
    UInt(double _o) : value((unsigned int)_o) {}
    ...
    UInt operator++(int) { UInt _t(*this); value++; return _t; }
    UInt& operator--()    { --value; return *this; }
    UInt operator--(int) { UInt _t(*this); value--; return _t; }
...
// Allows UInt to be used as key in std::unordered_map / std::unordered_set
namespace std
{
    template<>
    struct hash<UInt>
    {
        std::size_t operator()(const UInt& _v) const noexcept
        {
            return std::hash<unsigned int>{}(static_cast<unsigned int>(_v));
        }
    };
}
