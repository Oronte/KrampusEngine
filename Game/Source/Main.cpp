...
void StartGame()
{
	...
	_level->SpawnActor<Player>(RectangleShapeData())->transform.position = FVector2(800.0f);
	Actor* _actor = _level->SpawnActor<Player>(RectangleShapeData());
	_actor->transform.position = FVector2(1500.0f, 400.0f);
	_actor->transform.rotation = Angle(45, true);
	_level->SpawnActor<Player>(CircleShapeData())->transform.position = FVector2(200.0f);
